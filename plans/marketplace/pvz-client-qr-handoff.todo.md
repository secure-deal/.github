...
- [ ] `PVZ_HANDOFF_SECRET` (or document reuse of `JWT_SECRET`) in env / `Envs`
- [ ] `PvzHandoffTokenService` — sign/verify, `typ: 'pvz_handoff'`, TTL 30m, `jti`; verify checks Redis active `jti`
- [ ] Redis on `POST /token`: `SET pvz:handoff:active:{clientId} {jti} EX 1800` (overwrites → invalidates previous QR)
- [ ] Response DTO: `handoff_token`, `expires_at`, `pending_pickups_count` (no `qr_url`)
- [ ] Error codes: `PVZ_HANDOFF_TOKEN_INVALID`, `PVZ_HANDOFF_TOKEN_EXPIRED`, `PVZ_HANDOFF_NO_PICKUPS`, `PVZ_HANDOFF_DELIVERY_NOT_ELIGIBLE`
- [ ] Module `client/pvz-handoff` — controller, service, `api.decorators.ts`
- [ ] `POST /client/pvz-handoff/token` — only authenticated client; optional check for pending `at_pickup_point`

### Backend: employee resolve & confirm

- [ ] SQL `getClientDeliveriesAtEmployeeWarehouseSql` in `warehouse-ops.sql` + `npm run sql:generate`
- [ ] `POST /employee/warehouse-ops/pvz-handoff/resolve` — verify token, warehouse filter, response DTO
- [ ] `POST /employee/warehouse-ops/pvz-handoff/confirm` — `{ delivery_ids[] }`, reuse `transition` per id in transaction
- [ ] `client_hint` helper (masked phone / profile name) for employee UI only

### Backend: tests

- [ ] Unit: token service (valid, expired, wrong typ)
- [ ] Integration: token → resolve → confirm → `complete-delivery` (extend lifecycle spec)
...
- [ ] `marketplace-employee-app` — scan QR mode + list deliveries + confirm button
- [ ] Locales ru/tj for static labels (if UI strings added)

## Phase 4.1 — Hardening & UX

- [ ] Redis: one-time `jti` on resolve (optional replay protection)
